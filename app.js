document.addEventListener("DOMContentLoaded", () => {
  ...
  // -----------------------
  const goalTimeSec = 8 * 60; // 8 min goal for 2km
  ...
  const plan = {
    Monday: {
      ...
      main:[{text:`400 m @ ${goalKph} kph`, reps:6, duration:Math.round((0.4/goalDistance)*goalTimeSec), rest:20}],
      mobility:["Hip flexor stretch – 60 sec","Calf stretch – 60 sec"]
    },
    Tuesday:{
      title:"Tempo Run",
      explain:`Sustained 25 min at ${(goalKph*0.85).toFixed(1)} kph`,
    ...
    Wednesday:{
      title:"Recovery",
      explain:"Easy aerobic run 20 min",
    ...
    Thursday:{
      ...
      mainVO2:[{text:`500 m @ ${(goalKph*1.1).toFixed(1)} kph`, reps:5, duration:Math.round((0.5/goalDistance)*goalTimeSec*0.9), rest:120}],
      mainHill:[{text:`Hill sprint 60 m`, reps:6, duration:15, rest:60}],
      mobility:["Quad stretch – 60 sec"]
    },
    Friday:{
      title:"Endurance + Strides",
      explain:`35 min easy run + 4 × 1 min strides @ 12-14 kph`,
    ...
    Saturday:{
      ...
      warmup:["10 min easy jog"],
      main:[{text:`1 km @ ${(goalKph*0.95).toFixed(1)} kph`, duration:300},
  ...
  resetSession.addEventListener("click", ()=>{
    ...
    document.querySelectorAll(".repRow .progressBar").forEach(pb=>pb.style.width="0%");
  });
  ...
  function renderMainBlocks(day,type){
    ...
    mainBlock.querySelectorAll(".restRow").forEach(e=>e.remove());
  ...
  function renderCalendar(){
    ...
    for(let i=1;i<=daysInMonth;i++){
      ...
      if(completionData[i] && completionData[i].completed){
        ...
        div.title=`Pre: ${completionData[i].pre}, Post: ${completionData[i].post}`;
      }
