...
const plan = {
  Monday: {
    title:"Intervals",
    explain:`Short, fast repetitions at target speed of ${goalKph} kph`,
    warmup:["10 min easy jog","Dynamic mobility"],
    main:[{text:`400 m @ ${goalKph} kph (full effort)`,reps:6,duration:Math.round((0.4/goalDistance)*goalTimeSec),rest:20}],
  ...
  Tuesday:{
    ...
    main:[{text:"Tempo run",duration:25*60},{text:`3 × 100 m relaxed strides @ ${(goalKph*0.85).toFixed(1)} kph`,reps:3,duration:60,rest:30}],
    mobility:["Hamstring stretch – 60 sec"]
  },
  Wednesday:{
    title:"Recovery",
    explain:"Low intensity aerobic work.",
    warmup:["5 min brisk walk"],
    main:[{text:"Easy 20 min run @ 9-10 kph",duration:20*60}],
  ...
  Thursday:{
    ...
    mainVO2:[{text:`500 m slightly faster than goal (~${(goalKph*1.1).toFixed(1)} kph)`,reps:5,duration:Math.round((0.5/goalDistance)*goalTimeSec*0.9),rest:120}],
    mainHill:[{text:`Hill sprint 60 m`,reps:6,duration:15,rest:60}],
    mobility:["Quad stretch – 60 sec"]
  },
  Friday:{
    title:"Endurance + Strides",
    explain:`Easy aerobic run ~35 min + 4 × 1 min strides @ 12-14 kph.`,
    warmup:["5–10 min easy jog"],
    main:[{text:"Easy 35 min run @ 10 kph",duration:35*60},{text:"Strides 1 min each @ 12-14 kph",reps:4,duration:60,rest:30}],
  ...
  Saturday:{
    ...
    warmup:["10 min easy jog"],
    main:[{text:`1 km @ ${(goalKph*0.95).toFixed(1)} kph`,duration:300},{text:"Recovery 2 min",duration:120},{text:`500 m @ ${goalKph} kph`,duration:150},{text:`2 × 400 m fast finish @ ${(goalKph*1.05).toFixed(1)} kph`,reps:2,duration:120,rest:60}],
...
function renderDay(day){
  ...
  warmupList.innerHTML=""; data.warmup.forEach(item=>{
    const li=document.createElement("li"); li.textContent=item; warmupList.appendChild(li);
  });

  mainBlock.innerHTML="";
  if(day==="Thursday"){
    const container=document.createElement("div"); container.className="thursdayDropdown"; container.style.background="#222"; container.style.padding="10px"; container.style.borderRadius="12px"; container.style.marginBottom="10px"; container.style.border="2px solid var(--accent)";
    const label=document.createElement("div"); label.textContent="Select Thursday Workout:"; label.style.color="var(--accent)"; label.style.fontWeight="bold"; label.style.marginBottom="6px"; container.appendChild(label);
    const dropdown=document.createElement("select"); dropdown.innerHTML=`<option value="VO2">VO₂ Max</option><option value="Hill">Hill Sprint</option>`;
    dropdown.addEventListener("change",e=>{renderMainBlocks(day,e.target.value);});
    container.appendChild(dropdown); mainBlock.appendChild(container); renderMainBlocks(day,"VO2");
...
function renderMainBlocks(day,type){
  ...
  let sets=plan[day]; let mainData=sets.main;
  ...
  mainData.forEach(set=>{
    ...
    for(let i=1;i<=reps;i++){
      ...
      const content=document.createElement("div"); content.style.position="relative"; content.style.display="flex"; content.style.justifyContent="space-between"; content.style.alignItems="center"; content.style.padding="8px 12px";
      content.innerHTML=`<span>${set.text} (Rep ${i}/${reps})</span><span class="timerSpan" data-duration="${set.duration}">${formatTime(set.duration)}</span><input type="checkbox"/>`;
      div.appendChild(content); mainBlock.appendChild(div);
